...
specified performance metrics with ±0.001 tolerance for MSE and Spearman correlation.
...
All data from cluster logs and specifications
...
class ChromeCRISPRValidator:
    ...
    Validates each model against specified performance metrics with
    ...
    def __init__(self, models_base_path: str = "../models"):
        ...
        # Performance specifications - values from research
    ...
    def validate_model(self, model_path: Path, model_name: str) -> ValidationResult:
        """
        Validate a single model against performance specifications.
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Check if this is a hybrid model that needs GC content
                if 'CNN_GRU' in str(model_path) or 'CNN_LSTM' in str(model_path) or 'CNN_BiLSTM' in str(model_path):
                    # Generate GC content for hybrid models
                    gc_content = torch.rand(inputs.size(0), 1)  # Random GC content
    ...
    def validate_all_models(self) -> List[ValidationResult]:
        ...
        if passed_count == total_count:
            ...
            print("  Some models failed validation. Check individual results above.")
